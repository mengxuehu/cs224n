...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, [-1, embeddings.shape[1].value * embeddings.shape[2].value])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        self.w1 = xavier_weight_init()([x.shape[1].value, self.config.hidden_size])
        b1 = tf.get_variable("b1", shape=[self.config.hidden_size], initializer=tf.zeros_initializer())
        h = tf.nn.relu(tf.matmul(x, self.w1) + b1)

        self.w2 = xavier_weight_init()([self.config.hidden_size, self.config.n_classes])
        b2 = tf.get_variable("b2", shape=self.config.n_classes, initializer=tf.zeros_initializer())
        pred = tf.nn.relu(tf.matmul(h, self.w2) + b2)
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        loss += 0.5 * self.config.regularization_parameter * \
                tf.sqrt(tf.reduce_sum(self.w1 ** 2) + tf.reduce_sum(self.w2 ** 2))
        ### END YOUR CODE
        return loss
