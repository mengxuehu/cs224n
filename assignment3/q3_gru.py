...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        # gradients, vars_ = zip(*optimizer.compute_gradients(loss))
        # if self.config.clip_gradients:
        #     gradients, self.grad_norm = tf.clip_by_global_norm(gradients, clip_norm=self.config.max_grad_norm)
        # else:
        #     self.grad_norm = tf.global_norm(gradients)
        # train_op = optimizer.apply_gradients(zip(gradients, vars_))
        ...
        # self.grad_norm = tf.constant(0.1)
