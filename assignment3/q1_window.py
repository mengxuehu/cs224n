...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        sentence = [start for _ in range(window_size)] + sentence + [end for _ in range(window_size)]
        n_words = window_size * 2 + 1
        temp = [sentence[i:i + len(labels)] for i in range(n_words)]
        windowed_data.extend(list(zip(np.concatenate(temp, axis=1).tolist(), labels)))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, (-1, self.input_placeholder.shape[1].value * embeddings.shape[2].value))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        w = tf.get_variable("W", shape=[x.shape[1].value, self.config.hidden_size],
                            initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable("b1", shape=[self.config.hidden_size], initializer=tf.zeros_initializer())
        h = tf.nn.relu(tf.matmul(x, w) + b1, "ReLU")

        h_drop = tf.nn.dropout(h, keep_prob=dropout_rate, name="dropout")
