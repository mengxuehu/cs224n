...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        n_mask = max(max_length - len(sentence), 0)
        sentence = sentence[:max_length] + [zero_vector] * n_mask
        labels = labels[:max_length] + [zero_label] * n_mask
        mask = [True] * (max_length - n_mask) + [False] * n_mask
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        ce = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        loss = tf.reduce_mean(tf.boolean_mask(ce, self.mask_placeholder))
        ### END YOUR CODE
        return loss
