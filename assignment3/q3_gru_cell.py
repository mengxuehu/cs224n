...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            shape_u = [self.input_size, self.state_size]
            shape_w = [self.state_size, self.state_size]
            shape_b = [self.state_size]

            uz = tf.get_variable("U_z", shape=shape_u, initializer=tf.contrib.layers.xavier_initializer())
            wz = tf.get_variable("W_z", shape=shape_w, initializer=tf.contrib.layers.xavier_initializer())
            bz = tf.get_variable("b_z", shape=shape_b, initializer=tf.zeros_initializer())

            zt = tf.sigmoid(tf.matmul(inputs, uz) + tf.matmul(state, wz) + bz)

            ur = tf.get_variable("U_r", shape=shape_u, initializer=tf.contrib.layers.xavier_initializer())
            wr = tf.get_variable("W_r", shape=shape_w, initializer=tf.contrib.layers.xavier_initializer())
            br = tf.get_variable("b_r", shape=shape_b, initializer=tf.zeros_initializer())

            rt = tf.sigmoid(tf.matmul(inputs, ur) + tf.matmul(state, wr) + br)

            uo = tf.get_variable("U_o", shape=shape_u, initializer=tf.contrib.layers.xavier_initializer())
            wo = tf.get_variable("W_o", shape=shape_w, initializer=tf.contrib.layers.xavier_initializer())
            bo = tf.get_variable("b_o", shape=shape_b, initializer=tf.zeros_initializer())

            ot = tf.tanh(tf.matmul(inputs, uo) + tf.multiply(rt, tf.matmul(state, wo)) + bo)

            new_state = tf.multiply(zt, state) + tf.multiply(1 - zt, ot)
            ### END YOUR CODE ###
